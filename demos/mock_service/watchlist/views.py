...
from watchlist.models import User, Api, Log
...
def index():
    if request.method == 'POST':
        ...
        if not name or len(name) > 60 or not path or len(path) > 60  or len(response) > 60:
            flash('输入异常！')
            return redirect(url_for('index'))
        ...
        api = Api(name=name, path=path, response=response)
        db.session.add(api)
        db.session.commit()
        flash('添加成功')
        return redirect(url_for('index'))

    api_list = Api.query.order_by(Api.id.asc()).all()
